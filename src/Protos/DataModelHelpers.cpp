...
void fillPartModel(const IStorage & storage, const IMergeTreeDataPart & part, Protos::DataModelPart & part_model, bool ignore_column_commit_time)
{
    ...
    part_model.set_data_path_id(getPathIDFromDisk(part.))
    part_model.set_data_path_id(part.volume->getDefaultDisk()->getID());
    ...
    RPCHelpers::fillUUID(part.part_id, *(part_model.mutable_part_id()));
}
