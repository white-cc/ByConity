...
LimitSeekableReadBuffer::LimitSeekableReadBuffer(SeekableReadBuffer& in_,
    UInt64 offset, std::optional<UInt64> size, bool should_throw_):
        SeekableReadBuffer(nullptr, 0), in(in_), range_start(offset),
        range_end(std::nullopt), should_throw(should_throw_),
        underlying_cursor(0)
{
    if (size.has_value())
        range_end = range_start + size.value();
