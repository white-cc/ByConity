...
{
public:
    S3AttachPartsInfo(const StoragePtr& from_tbl_, const IMergeTreeDataPartsVector& former_parts_,
...
{
public:
    S3AttachMetaAction(const Context& context_, const TxnTimestamp& txn_id_,
        const StoragePtr& target_tbl, const S3AttachPartsInfo& parts_info_):
            IAction(context_, txn_id_), from_tbl(parts_info_.from_tbl),
            to_tbl(target_tbl), former_parts(parts_info_.former_parts),
    ...
    static void collectUndoResourcesForCommit(const UndoResources& resources,
        NameSet& part_names);
